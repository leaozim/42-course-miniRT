...
static int	identifier(char *line, t_bool reqrid_obj[])
{
	char	**tokens;
	...
	if (ft_strcmp(tokens[0], _AMBIENT_) == 0 && reqrid_obj[AMBIENT] == FALSE)
		return (check_id_a(tokens, reqrid_obj));
	else if (ft_strcmp(tokens[0], _CAMERA_) == 0 && reqrid_obj[CAMERA] == FALSE)
		return (check_id_c(tokens, reqrid_obj));
	else if (ft_strcmp(tokens[0], _LIGHT_) == 0 && reqrid_obj[LIGHT] == FALSE)
		return (check_id_l(tokens, reqrid_obj));
