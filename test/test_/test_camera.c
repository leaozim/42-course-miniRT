...
void	test_print_three_spheres(void)
{
	...
	set_transform(middle, translation(-0.5, 0.99, -0.9));
	middle->material.color = formatted_color(middle->material.color, 0.1, 1, 0.5);
	middle->material.diffuse = 0.7;
	middle->material.specular = 0.3;

	right = create_sphere();
	aux1 = translation(0.3, 2, -1.1);
	set_transform(right, multiply_matrix(aux1, scaling(0.5, 0.5, 0.2)));
	right->material.color = formatted_color(right->material.color, 0.5, 1, 0.1);
	right->material.diffuse = 0.7;
	right->material.specular = 0.3;

	left = create_sphere();
	aux1 = translation(-1.3, 1.92, -1.35);
	set_transform(left, multiply_matrix(aux1, scaling(0.5, 0.5, 0.2)));
	left->material.color = formatted_color(left->material.color, 0.5, 1, 0.1);
	...
	camera = init_cam(500, 500, radians(180 / 3));
	...
	win = mlx_new_window(canvas.mlx, (int)round(camera.hsize), (int)round(camera.vsize), "test");
